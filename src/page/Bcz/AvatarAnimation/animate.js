...
function getEasingFunction(easing) {
    // 将 easing 转换为驼峰命名
    var easingCamelCase = convertToCamelCase(easing);
    ...
    if (easing.startsWith('cubic-bezier')) {
        // 使用正则表达式解析参数
        var match = easing.match(/cubic-bezier\(([^,]+),([^,]+),([^,]+),([^,]+)\)/);
        if (match) {
            var p1_1 = [+match[1], +match[2]];
            var p2_1 = [+match[3], +match[4]];
            // 检查参数是否都是有效的数字
            if (p1_1.some(isNaN) || p2_1.some(isNaN)) {
                throw new Error('Invalid arguments for cubic-bezier');
            }
            // 检查 x 值是否在 0 到 1 之间
            if (p1_1[0] < 0 || p1_1[0] > 1 || p2_1[0] < 0 || p2_1[0] > 1) {
                throw new Error('The x values of the control points must be between 0 and 1');
            }
            return function (progress) {
                return calculateBezier(progress, p1_1, p2_1);
            };
        }
    }
...
function parseColor(color) {
    ...
    if (result) {
        ...
        if (r > 255 || r < 0 || g > 255 || g < 0 || b > 255 || b < 0) {
            throw new Error("Invalid color value: ".concat(color, " hints: 0 <= r, g, b <= 255"));
        }
    ...
    if (result) {
        ...
        if (r > 255 || r < 0 || g > 255 || g < 0 || b > 255 || b < 0 || a > 1 || a < 0) {
            throw new Error("Invalid color value: ".concat(color, " hints: 0 <= r, g, b <= 255, 0 <= a <= 1"));
        }
    ...
    if (result) {
        ...
        if (h > 360 || h < 0 || s > 100 || s < 0 || l > 100 || l < 0) {
            throw new Error("Invalid color value: ".concat(color, " hints: 0 <= h <= 360, 0% <= s, l <= 100%"));
        }
        var hsla = hslToRgb(h / 360, s / 100, l / 100);
        return hsla;
    }
    // 处理hsla(...)
    ...
    if (result) {
        ...
        if (h > 360 || h < 0 || s > 100 || s < 0 || l > 100 || l < 0 || a > 1 || a < 0) {
            throw new Error("Invalid color value: ".concat(color, " hints: 0 <= h <= 360, 0% <= s, l <= 100%, 0 <= a <= 1"));
        }
        var rgb = hslToRgb(h / 360, s / 100, l / 100);
        return __assign(__assign({}, rgb), { a: a });
    }
    throw new Error("Invalid color value ".concat(color));
}
function interpolateColor(startColor, endColor, progress) {
    var startHsla = rgbaToHsla(parseColor(startColor));
    var endHsla = rgbaToHsla(parseColor(endColor));
    // 计算插值后的 HSLA 值
    var h = startHsla[0] + progress * (endHsla[0] - startHsla[0]);
    var s = startHsla[1] + progress * (endHsla[1] - startHsla[1]);
    var l = startHsla[2] + progress * (endHsla[2] - startHsla[2]);
    var a = startHsla[3] + progress * (endHsla[3] - startHsla[3]);
    // 将 HSLA 值转换回颜色字符串
...
var animate = function (partialOptions, startState, endState, updateFunc) {
    if (partialOptions === void 0) { partialOptions = {}; }
    var options = Object.assign({}, defaultOptions, partialOptions);
    var easingFunction = getEasingFunction(options.easing);
    var cancelled = false;
    var startTime = performance.now() + options.delay;
    ...
    function animation() {
        ...
        var rawProgress = Math.min(elapsed / options.duration, 1);
